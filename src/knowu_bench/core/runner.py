...
def _execute_single_task(
    ...
    if hasattr(agent, "set_task_context"):
        agent.set_task_context(task_name, env=env)

    step = 0
    obs = env.initialize_task(task_name=task_name)
    agent.initialize(task_goal)
    if hasattr(agent, "get_memrl_plan"):
        try:
            traj_logger.log_memrl_plan(agent.get_memrl_plan())
        except Exception:
            logger.exception("Failed to persist MemRL plan for {}", task_name)
    actions = []
    while True:
        step += 1
...
def _wait_for_env_health(
    env: AndroidEnvClient,
    *,
    timeout_seconds: int = 180,
    interval_seconds: int = 10,
) -> bool:
    """Wait until the backend reports a healthy emulator before starting a task."""
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            logger.warning(
                "Environment {} failed controller initialization during health wait: {}",
                env.base_url,
                e,
            )

        if env.health():
            logger.info("Environment {} is healthy before task start", env.base_url)
            return True

        remaining = max(0, int(deadline - time.time()))
        logger.warning(
            "Environment {} is not healthy yet (attempt {}, {}s remaining); waiting {}s",
            env.base_url,
            attempt,
            remaining,
            interval_seconds,
    ...
    **kwargs,
    ...
    try:
        with logger.contextualize(thread_id=thread_id, container_name=container_name):
            ...
            while True:
                try:
                    if not _wait_for_env_health(env):
                        raise RuntimeError("Device is not healthy after pre-task health wait")
