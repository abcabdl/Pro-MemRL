...
DEFAULT_MAX_STEP = 15
ASK_USER_TRANSPORT_ERROR_PREFIX = "__ASK_USER_TRANSPORT_ERROR__:"
...
class AndroidEnvClient:
    ...
    def execute_action(self, action: JSONAction) -> Observation:
        ...
        if action.action_type == "ask_user":
            if not response.ok:
                ask_user_response = (
                    f"{ASK_USER_TRANSPORT_ERROR_PREFIX} status={response.status_code} "
                    f"body={response.text}"
                )
                logger.warning(
                    "ask_user failed at environment transport layer: {}",
                    ask_user_response,
                )
                ...
                try:
                    ...
                    ask_user_response = (
                        f"{ASK_USER_TRANSPORT_ERROR_PREFIX} invalid_json body={response.text}"
                    )
                    logger.warning(
                        "ask_user returned malformed JSON: {}",
                        ask_user_response,
                    )
