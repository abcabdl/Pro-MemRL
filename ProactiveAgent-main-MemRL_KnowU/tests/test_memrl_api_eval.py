...
from eval.run_memrl_api_eval import run_api_eval  # noqa: E402


def test_run_memrl_api_eval_smoke(tmp_path: Path) -> None:
    bootstrap_episode = {
        "memory_id": "b1",
        "sample_id": "b1",
        "source": "judge_stage2",
    ...
    async def fake_responder(messages: list[dict[str, str]]) -> dict[str, str]:
        assert messages[0]["role"] == "system"
        return {
            "Purpose": "The user is debugging.",
            "Thoughts": "A proactive suggestion may help.",
            "Proactive Task": "Inspect the traceback.",
            "Response": "I can help inspect the traceback.",
            "raw": "{}",
        }
    ...
    assert pred[0]["other_infomation"]["Decision"]["commitment_level"] >= 1
