...
from eadp import RUNTIME_FEEDBACK, normalize_operation
from memrl import record_feedback_payload
from personalization import default_state_path, load_runtime_state, record_runtime_feedback
...
def _memrl_enabled() -> bool:
    return os.getenv("ACTIVEAGENT_MEMRL_ENABLED", "false").lower() in ("1", "true", "yes", "on")


def _memrl_state_dir() -> str:
    return os.getenv(
...
def record_memrl_feedback(reaction: str) -> None:
    if not _memrl_enabled():
        return
    try:
        state_path = os.getenv("ACTIVEAGENT_PERSONALIZATION_STATE", str(default_state_path()))
        payload = load_runtime_state(state_path)
        ...
        record_feedback_payload(
            last_feedback,
            reaction=reaction,
            state_dir=_memrl_state_dir(),
            bootstrap_path=_memrl_bootstrap(),
        ...
        logger.warning(f"Failed to record MemRL feedback: {exc}")
...
class ActionListener(object):
    ...
    def send_data(self) -> dict:
        ...
        apps_showed = set(
            event.get("data", {}).get("app")
            for event in event_windows
            if event.get("data", {}).get("app") is not None
        )
        attached_info = {}

        if event_vscode and ("Code.exe" in apps_showed or "Code" in apps_showed):
            attached_info["vscode"] = event_vscode

        for app in self.chrome_apps:
            if app in apps_showed and event_chrome:
...
class Executor(Trigger):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.base_url = None
        # Send a plain mirror notification to improve visibility in demo environments.
        self.mirror_plain_toast = os.getenv("ACTIVEAGENT_MIRROR_PLAIN_TOAST", "true").lower() in ("1", "true", "yes", "on")
        # GUI popup fallback for environments where Windows notifications are blocked.
        self.tk_popup_fallback = os.getenv("ACTIVEAGENT_TK_POPUP_FALLBACK", "true").lower() in ("1", "true", "yes", "on")
    ...
    def _show_tk_popup(self, message: str, on_accept, on_dismiss):
        try:
            import tkinter as tk
        except Exception as e:
            logger.error(f"tk popup fallback unavailable: {e}")
            return

        def _run_popup():
            root = tk.Tk()
            root.title("ActiveAgent")
            root.attributes("-topmost", True)
            root.resizable(False, False)
            root.geometry("420x170+1480+840")

            label = tk.Label(root, text=message, justify="left", wraplength=390)
            label.pack(padx=12, pady=(12, 8), fill="both", expand=True)
            ...
            tk.Button(btn_frame, text="Accept", width=12, command=_accept).pack(side="left", padx=8)
            tk.Button(btn_frame, text="Dismiss", width=12, command=_dismiss).pack(side="left", padx=8)

            # Auto close in case user ignores it.
    ...
    def send(self):
        ...
        if sys.platform == "win32":
            ...
            def activated_callback(activatedEventArgs: Optional[ToastActivatedEventArgs] = None):
                ...
                RUNTIME_FEEDBACK.record_accept()
                record_runtime_feedback("accept")
                record_memrl_feedback("accept")
                ...
                if normalize_operation(func_name) == "notify_only":
                    logger.info("notify_only accepted; skip tool execution.")
                    return
...
Please accomplish the proposal raised by the agent.""".format(target = infos, content = response['content'])
            ...
            def dismiss_callback(dismissedEventArgs = None):
                global status
                status = 'The user REJECT the last proposal from you. consider propose in another way or do not disturb the user.'
                RUNTIME_FEEDBACK.record_reject()
                record_runtime_feedback("dismiss")
                record_memrl_feedback("dismiss")

            # For now, we will assemble a toast.
            global interactable, newToast
            toast_text = self.response.get("Response")
            proactive_task = self.response.get("Proactive_Task")

            if toast_text is None or toast_text == "null" or str(toast_text).strip() == "":
                toast_text = "I have a suggestion for your current activity. Click to let me help."

            # Create the toast and try to keep interactive elements optional.
            newToast = Toast(
                text_fields=[toast_text],
                on_activated=activated_callback,
                on_dismissed=dismiss_callback,
            )

            # Add a selectable assistance item only when task text is valid.
            if proactive_task is not None and proactive_task != "null" and str(proactive_task).strip() != "":
                try:
                    toastSelections = (
                        ToastSelection(selection_id="_", content=str(proactive_task)),
                    )
                    selectionBoxInput = ToastInputSelectionBox(
                        "action", "My Assistance", toastSelections, default_selection=toastSelections[0]
                    )
                    newToast.AddInput(selectionBoxInput)
                except Exception as e:
                    logger.warning(f"Failed to attach interactive selection box, fallback to plain toast: {e}")

            try:
                interactable.show_toast(newToast)
                logger.info(f"Toast dispatched: {toast_text}")
                if self.mirror_plain_toast:
                    response_toaster.show_toast(Toast(text_fields=[toast_text]))
                    logger.info(f"Mirror plain toast dispatched: {toast_text}")
            except Exception as e:
                logger.error(f"Failed to show interactive toast, fallback to plain toast: {e}")
                fallback_toast = Toast(text_fields=[toast_text])
                response_toaster.show_toast(fallback_toast)
                logger.info(f"Fallback toast dispatched: {toast_text}")

            if self.tk_popup_fallback:
                logger.info("Tk popup fallback dispatched.")
                self._show_tk_popup(toast_text, activated_callback, dismiss_callback)
