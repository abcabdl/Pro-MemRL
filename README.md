...
`gelab_agent`, `general_e2e`, `gui_owl_1_5`, `mai_ui_agent`, `memrl_knowu`, `planner_executor`, `qwen3.5`, `qwen3vl`, `seed_agent`, `ui_venus_agent`

You can also pass a custom Python file path to `--agent-type` as long as it defines a class derived from `BaseAgent`.

### MemRL + KnowU Routine Adapter

This checkout can use the embedded `ProactiveAgent-main-MemRL_KnowU` runtime as a memory layer for KnowU routine tasks.

Build a non-test training memory bundle from user profiles and logs first:

```bash
uv run python scripts/build_knowu_memrl_bundle.py \
  --source profile-task-matrix \
  --output-dir artifacts/memrl_knowu_profile_task_train_bundle \
  --target-count 300
```

This writes `artifacts/memrl_knowu_profile_task_train_bundle/memrl_episodes.jsonl` and a compatible `snapshot/memrl_snapshot.jsonl` without reading KnowU task expectations. Then run routine evaluation with:

```bash
export KNOWU_MEMRL_BOOTSTRAP=artifacts/memrl_knowu_profile_task_train_bundle/memrl_episodes.jsonl

uv run mw eval \
  --agent-type memrl_knowu \
  --task ALL \
  --task-tags routine \
  --user developer \
  --model-name your-model-name \
